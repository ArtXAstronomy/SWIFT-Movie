...
/**
 * @brief Un-skips all the imaging tasks associated with a given cell.
 *
 * @param c the #cell.
 * @param s the #scheduler.
 */
void cell_unskip_imaging_tasks(struct cell *c, struct scheduler *s) {
   ...
   * cell). */
  if (c->image_data.imaging != NULL) {
    /* Only activate the imaging task if it is on the local node. */
    if (c->nodeID == nodeID) {
      scheduler_activate(s, c->image_data.imaging);

      /* Also unskip the imaging collection task at the top level. */
      scheduler_activate(s, c->top->image_data.imaging_collect);
    }
  }
}
