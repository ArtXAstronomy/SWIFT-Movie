...
void engine_allocate_foreign_particles(struct engine *e, const int fof) {
  ...
  if (e->verbose) {
    ...
    if ((s->size_parts_foreign - old_size_parts_foreign) > 0 ||
      ...
      message("Re-allocations %zd/%zd/%zd/%zd/%zd part/gpart/spart/bpart/sink "
              "(%zd/%zd/%zd/%zd/%zd MB)",
...
int engine_estimate_nr_tasks(const struct engine *e) {
  ...
  if (ncells > 0)
    tasks_per_cell = ceilf(ntasks / ncells);
...
int engine_prepare(struct engine *e) {
  ...
  if (e->policy & engine_policy_fof && e->forcerebuild && !e->forcerepart &&
    ...
    if (e->dump_catalogue_when_seeding)
...
void engine_run_rt_sub_cycles(struct engine *e) {
  ...
  if (nr_rt_cycles < 0) {
    error("Got negative nr of sub-cycles??? ti_rt_end_min = %lld ti_current = "
  ...
  for (int sub_cycle = 1; sub_cycle < nr_rt_cycles; ++sub_cycle) {
    ...
    if (e->nodeID == 0) {
      ...
      printf(" [rt-sc] %-4d %12e %11.6f %11.6f %13e %4d %4d %12lld %12s %12s "
...
void engine_init_particles(struct engine *e, int flag_entropy_ICs,
  ...
  if (e->policy & engine_policy_stars)
    stars_exact_density_compute(e->s, e);
  ...
  if (e->policy & engine_policy_sinks)
    sink_exact_density_compute(e->s, e);
...
void engine_compute_gui_data(const struct engine *e, double *sfrd,
  ...
  for (int i = 0; i < s->nr_sparts; i++) {
    ...
    *stellar_mass += sparts[i].mass;
    ...
    if (sparts[i].birth_time * 1e6 >= t_then) {
      /* Convert to SFRD (Msun / Gyr) */
...
int engine_write_gui_data(const struct engine *e) {
  ...
  fprintf(f,
          ...
          e->updates, e->g_updates, e->s_updates, e->b_updates,
          e->wallclock_time, percentage,
          cosmology_get_time_since_big_bang(e->cosmology, e->cosmology->a) *
              1000,
          stellar_mass, sfrd);
 ...
 * @param image_data the #image_common_data of this run.
 */
void engine_init(
...
void engine_clean(struct engine *e, const int fof, const int restart) {
  ...
  output_list_clean(&e->output_list_imaging);
  ...
  if (restart) {
    ...
    if (e->output_list_imaging)
      free((void *)e->output_list_imaging);
#ifdef WITH_CSDS
    if (e->policy & engine_policy_csds)
      free((void *)e->csds);
#endif
